...
namespace gui {
	...
	void OnGUI(uintptr_t rcx)
	{
		...
		if (event_type == rust::classes::EventType::Repaint) {
			{
				/*gui::Label(rust::classes::Rect{ 10 + 2.0f + 1, 20 - 4 + 1, 300, 50 }, _(L"<Scum>"), gui::Color(0, 0, 0, 120), false, 24);
				gui::Label(rust::classes::Rect{ 10 + 2.0f, 20 - 4, 300, 50 }, _(L"<Scum>"), gui::Color(255, 255, 255, 220), false, 24);*/
				static int cases = 0;
				...
				const float ScreenWidth = 2560;//screenresolution
				const float ScreenHeight = 1440;
				gui::Label(rust::classes::Rect{ 10 + 2.0f + 1, 20 - 4 + 1, 300, 50 }, _(L"<Scum>"), gui::Color(r, g, b, 120), false, 24);
				gui::Label(rust::classes::Rect{ 10 + 2.0f, 20 - 4, 300, 50 }, _(L"<Scum>"), gui::Color(r, g, b, 220), false, 24);
				if (settings::misc::Crosshair) {
					...
					gui::horizontal_line(vector2{ (float)(ScreenWidth / 2), (float)(ScreenHeight / 2) }, 6.f, gui::Color(1, 0, 0, 1));
					gui::horizontal_line(vector2{ (float)(ScreenWidth / 2), (float)(ScreenHeight / 2) }, 6.f, gui::Color(1, 0, 0, 1));
					gui::vertical_line(vector2{ (float)(ScreenWidth / 2), (float)(ScreenHeight / 2) }, 6.f, gui::Color(1, 0, 0, 1));
					gui::vertical_line(vector2{ (float)(ScreenWidth / 2), (float)(ScreenHeight / 2) }, 6.f, gui::Color(1, 0, 0, 1));
				}
				esp::start();
			}
		...
		if (open) {
			{
				...
				gui::Label(rust::classes::Rect{ menu_pos.x + 2.0f + 1, menu_pos.y - 4 + 1, menu_size.x, 30 }, _(L"<SCUM>"), gui::Color(r, g, b, 120), true, 16);
				gui::Label(rust::classes::Rect{ menu_pos.x + 2.0f + 1, menu_pos.y - 4 + 1, menu_size.x, 30 }, _(L"<SCUM>"), gui::Color(r, g, b, 220), true, 16);
				...
				case 0:
					...
					if (settings::weapon::thick_bullet) {
						Slider(menu_pos, il2cpp::methods::new_string(string::format(("%s %d"), _("Thickness:"), (int)settings::weapon::thickness)), pos, { 0, 3 }, settings::weapon::thickness, weapon_tab);
					}
				...
				case 2:
					...
					button(event_type, menu_pos, pos, mouse_pos, _(L"Silent Walk(Press X)"), &settings::misc::silentwalk, misc_tab);
					...
					button(event_type, menu_pos, pos, mouse_pos, _(L"instantrevive"), &settings::misc::instant_revive, misc_tab);
					button(event_type, menu_pos, pos, mouse_pos, _(L"interactivedebug"), &settings::misc::interactive_debug, misc_tab);
					button(event_type, menu_pos, pos, mouse_pos, _(L"Eyeoffset"), &settings::misc::eyeoffset, misc_tab);
					if (settings::misc::auto_lock) {
						Slider(menu_pos, il2cpp::methods::new_string(string::format(_("%s %d"), _("Code:"), (int)settings::misc::code_lock_code)), pos, { 1000, 9999 }, settings::misc::code_lock_code, misc_tab);
					...
					if (settings::misc::brightnight) {
						Slider(menu_pos, il2cpp::methods::new_string(string::format(("%s %d"), _("Star Brightness:"), (int)settings::misc::staramount)), pos, { 0, 2000 }, settings::misc::staramount, misc_tab);
					}
					break;
				case 3:
					...
					if (settings::visuals::materials) {
						...
						button(event_type, menu_pos, pos, mouse_pos, _(L"  - Food"), &settings::visuals::food, other_esp);
						button(event_type, menu_pos, pos, mouse_pos, _(L"  - Berry"), &settings::visuals::berry, other_esp);
						button(event_type, menu_pos, pos, mouse_pos, _(L"  - Animal"), &settings::visuals::animal, other_esp);
						button(event_type, menu_pos, pos, mouse_pos, _(L"  - Crates"), &settings::visuals::lootesp, other_esp);
						button(event_type, menu_pos, pos, mouse_pos, _(L"  - KeyCard"), &settings::visuals::card, other_esp);
						button(event_type, menu_pos, pos, mouse_pos, _(L"  - Barrels"), &settings::visuals::barrels, other_esp);
					}
					...
					if (settings::visuals::vehicle) {
						...
						button(event_type, menu_pos, pos, mouse_pos, _(L"  - mini1"), &settings::visuals::mini1, other_esp);
						button(event_type, menu_pos, pos, mouse_pos, _(L"  - mini2"), &settings::visuals::mini2, other_esp);
						button(event_type, menu_pos, pos, mouse_pos, _(L"  - Hose"), &settings::visuals::horse, other_esp);
					}
