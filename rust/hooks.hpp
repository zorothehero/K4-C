...
namespace hooks {
	namespace orig {
		...
		//static auto hitsound = reinterpret_cast<void(*)(DWORD64)>(reinterpret_cast<uintptr_t*>(il2cpp::method(_("BaseEntity"), _("OnAttacked"), 1, _(""), _(""))));
...
#pragma optimize("", on)
	...
	/*void hk_hitty(DWORD64 info) {
		if (settings::misc::Hitsound)
			LI_FIND(PlaySoundA)("C:\\hitsound\\Metal.wav", NULL, SND_ASYNC | SND_FILENAME);   // hitsound
		return hooks::orig::hitsound(info);
