		...
		if (open) {
			{
				...
				case 0:
					...
					button(event_type, menu_pos, pos, mouse_pos, _(L"Rapid fire"), &settings::weapon::rapidfire, weapon_tab);
					break;
				case 1:
...
#define newstring new_string
							...
							if (settings::visuals::hotbar_esp) {
								gui::fill_box({ 658 + info_y, (unity::get_height() * 0.85f), 90, 90 }, { 0, 0, 0, 0.12 });

								gui::methods::set_color({ 1, 1, 1, 1 });
								gui::methods::DrawTexture({ 658 + info_y, (unity::get_height() * 0.85f), rect.wid / 4.0f, rect.hei / 4.0f }, texture);
